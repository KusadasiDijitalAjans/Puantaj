# Puantaj V1.2.1 - Windows release build script
...
try {
    ...
    if ($null -eq $iscc) {
        ...
        $localCandidate = Join-Path $env:LOCALAPPDATA "Programs\Inno Setup 6\ISCC.exe"
        if (Test-Path $candidate) { $iscc = $candidate } elseif (Test-Path $localCandidate) { $iscc = $localCandidate } else {
    ...
    Write-Host "  Setup:            dist\Puantaj_Setup_v1.2.1.exe"
}
finally {
    Pop-Location
}
